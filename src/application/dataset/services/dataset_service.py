...
from src.domain.exceptions.global_exceptions import PathEmptyException
...
def convert_dataset(
    ...
    max_images = parameters.max_images
    force_channels = parameters.force_channels
    subfolders_as_labels = parameters.subfolders_as_labels
    ...
    if dest == '':
        raise PathEmptyException(
            name=PathEmptyException.__name__, message="dest output filename or directory must not be an empty string"
        )
    ...
    return {"Message": "Converted Dataset"}
