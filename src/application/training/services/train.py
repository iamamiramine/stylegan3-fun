...
import dnnlib
from application.training.models import training_loop
from src.domain.metrics import metric_main
from infrastructure.torch_utils import training_stats, custom_ops, gen_utils
...
def subprocess_fn(rank, c, temp_dir):
    infrastructure.dnnlib.util.Logger(file_name=os.path.join(c.run_dir, 'log.txt'), file_mode='a', should_flush=True)
...
def launch_training(c, desc, outdir, dry_run):
    infrastructure.dnnlib.util.Logger(should_flush=True)
...
def init_dataset_kwargs(data):
    try:
        ...
        dataset_obj = infrastructure.dnnlib.util.construct_class_by_name(**dataset_kwargs) # Subclass of training.dataset.Dataset.
