...
from application.metrics.models import metric_utils, metric_main
from src.infrastructure.torch_utils import training_stats, custom_ops, misc
from src.infrastructure.torch_utils.ops import conv2d_gradfix
...
def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose):
    ...
    with dnnlib.util.open_url(network_pkl, verbose=args.verbose) as f:
        network_dict = legacy_service.load_network_pkl(f)
        args.G = network_dict['G_ema'] # subclass of torch.nn.Module

    # Initialize dataset options.
    if data is not None:
        args.dataset_kwargs = dnnlib.EasyDict(class_name='src.application.training.models.dataset.ImageFolderDataset', path=data)
